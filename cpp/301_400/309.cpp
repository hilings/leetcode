...
public:
    int maxProfit(vector<int>& prices) {
        ...
        if (n < 2)
            return 0;
        vector<int> cool {0, 0};
        vector<int> sell {0, max(0, prices[1]-prices[0])};
        for (int i = 2; i < n; i++) {
            cool.push_back(max(cool[i-1], sell[i-1]));
            sell.push_back(prices[i]-prices[i-1] + max(sell[i-1], cool[i-2]));
        }
        return max(cool[n-1], sell[n-1]);
    }
    int maxProfit2(vector<int>& prices) {	// improvement from maxProfit
        int n = (int)prices.size();
        if (n < 2)
            return 0;
        int cool2 = 0, cool = 0, sell = max(0, prices[1]-prices[0]);
        for (int i = 2; i < n; i++) {
            int tmp = cool;
            cool = max(cool, sell);
            sell = prices[i]-prices[i-1] + max(sell, cool2);
            cool2 = tmp;
        }
        return max(cool, sell);
    }

    int maxProfit3(vector<int>& prices) {	// states machine
        int hold = INT_MIN, sell = 0, rest = 0;
        for (int p: prices) {
            int thold = hold, tsell = sell, trest = rest;
            hold = max(thold, tsell-p);	// do nothing to remain in, or buy in now to become 'buy' state
            sell = max(tsell, trest);	// do nothing to remain in, or from rest state to become 'sell' state
            rest = thold+p;	// sell now to become 'rest' state
        }
