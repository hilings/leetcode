...
//  leetcode
//
//  Created by Hang Zhang on 7/7/17.
...
class Solution {
    ...
    void helper(vector<vector<int>>& matrix, int i, int j, int mark, vector<pair<int,int>>& r) {
        int m = (int)matrix.size(), n = (int)matrix[0].size();
        if (i < 0 || i == m || j < 0 || j == n || visited[i][j] & mark) {
            return ;
        }
        visited[i][j] |= mark;
        if (visited[i][j] == 3)	r.push_back(pair<int,int> (i, j));
        if (j-1 >= 0 && matrix[i][j] <= matrix[i][j-1])	helper(matrix, i, j-1, mark, r);
        if (j+1 <  n && matrix[i][j] <= matrix[i][j+1])	helper(matrix, i, j+1, mark, r);
        if (i-1 >= 0 && matrix[i][j] <= matrix[i-1][j])	helper(matrix, i-1, j, mark, r);
        if (i+1 <  m && matrix[i][j] <= matrix[i+1][j])	helper(matrix, i+1, j, mark, r);
    }
public:
    vector<pair<int, int>> pacificAtlantic(vector<vector<int>>& matrix) {
        ...
        for (int i = 0; i < m; i++) {	// init
            helper(matrix, i, 0, 1, r);	// 1 for pacific
            helper(matrix, i, n-1, 2, r);	// 2 for atlantic
        }
        for (int j = 0; j < n; j++) {
            helper(matrix, 0, j, 1, r);
            helper(matrix, m-1, j, 2, r);
        }
...
int main(int arg, char *argv[]) {
    // insert code here...
    cout << "LeetCode 417. Pacific Atlantic Water Flow, C++ ...\n\n";
    Solution sol;
    ...
    vector<pair<int,int>> r = sol.pacificAtlantic(matrix);
