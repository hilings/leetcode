//
//  675_Cut_Off_Trees_for_Golf_Event.cpp
//  leetcode
//
...
class Solution {
    int path(vector<vector<int>>& forest, pair<int,int>& a, pair<int,int>& b) {
        int m = (int)forest.size(), n = (int)forest[0].size();
        vector<vector<bool>> visited (m, vector<bool> (n, false));
        queue<pair<int,int>> q;
        pair<int,int> sep (-1,-1);
        if (forest[a.first][a.second] > 0) {
            q.push(a);
        ...
        while (!q.empty()) {
            ...
            if (cur != sep) {
                if (i-1 >= 0 && forest[i-1][j] && !visited[i-1][j]) {
                    q.push(pair<int,int> (i-1,j));
                    visited[i-1][j] = true;
                }
                if (i+1 < m && forest[i+1][j] && !visited[i+1][j]) {
                    q.push(pair<int,int> (i+1,j));
                    visited[i+1][j] = true;
                }
                if (j-1 >= 0 && forest[i][j-1] && !visited[i][j-1]) {
                    q.push(pair<int,int> (i,j-1));
                    visited[i][j-1] = true;
                }
                if (j+1 < n && forest[i][j+1] && !visited[i][j+1]) {
                    q.push(pair<int,int> (i,j+1));
                    visited[i][j+1] = true;
                }
            } else if (!q.empty()) {
...
public:
    int cutOffTree(vector<vector<int>>& forest) {
        auto cmp = [&forest] (const pair<int,int>& a, const pair<int,int>& b) {
...
int main(int arg, char *argv[]) {
    // insert code here...
    cout << "LeetCode 675. Cut Off Trees for Golf Event, C++ ...\n\n";
    Solution sol;
