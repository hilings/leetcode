...
class Solution {
public:
    vector<int> largestDivisibleSubset0(vector<int>& nums) {	// brute force
        ...
        for (int a: nums) {
            ...
            for (int i = 0; i < cur_size; i++) {
                ...
                if (a % rr[i].back() == 0) {
                    ...
                    int j = rr[i].size()-1;
                    for (; j >= 0 && a % rr[i][j] != 0; j--) {
                        ;
                    }
                    if (j >= 0) {
                        vector<int> tv (rr[i].begin(), rr[i].begin()+j+1);
                        tv.push_back(a);
                        rr.push_back(tv);
                        appended = true;
                        cout << "adding  : ";
        ...
        for (int i = 0; i < rr.size(); i++) {
            ...
            if (rr[i].size() > length) {
                length = rr[i].size();
                index = i;
                cout << "updating length -> " << length << ", index -> " << index << '\n';
            }
        }
        return rr[index];
    }
    ...
    vector<int> largestDivisibleSubset(vector<int>& nums) {
        ...
        sort(nums.begin(), nums.end());
        vector<int> prevs (nums.size(), -1), lens (nums.size(), 1);
